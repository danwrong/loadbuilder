...
Dependency.prototype.fetchSource = function(loadbuilder) {
  this.source = fs.readFileSync(loadbuilder.options.docRoot + this.filename, 'utf-8');
};
Dependency.prototype.parseSource = function() {
  this.ast = jsp.parse(this.source);
};
Dependency.prototype.hintSource = function(loadbuilder) {
  jshint(this.source, {

  });
  if (jshint.errors.length) {
    loadbuilder.log(3, 'JSHint messages', jshint.errors);
  }
};
