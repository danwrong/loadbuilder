var fs     = require('fs'),
    jsp    = require('uglify-js').parser,
    jspro  = require("uglify-js").uglify,
    opts   = require('opts'),
    colors = require('colors');

function mixin(target, source) {
  Object.keys(source).forEach(function(key) {
    target[key] = source[key];
  });

  return target;
}

function extractDeps(argList) {
  return argList.map(function(item) {
    if (item[0] == 'string') {
      return item[1];
    }
  }).filter(  function(item) {
    return !(typeof item == 'undefined' || item == null);
  });
}

function isUsing(node) {
  return node[0] == 'call' && node[1][1] == 'using';
}

function isProvide(node) {
  return node[0] == 'call' && node[1][1] == 'provide';
}

function getSource(file) {
  var source = fs.readFileSync(file, 'utf-8');
  return jsp.parse(source);
}

function addToEnd(arr, item) {
  var index = arr.indexOf(item);
  if (index>=0) {
    arr.splice(index, 1);
  }
  arr.push(item);
}

var modulePath = 'modules/';

var fileObj = function(name, type, filename) {
  this.name = name;
  this.type = type || '';
  this.ast = [];
  this.filename = filename;
};

var matchers = {};
matchers['\\.js$'] = function(filename, options, dependencies) {
  // TODO: only add scripts to the stack if they can be loaded
  var file = new fileObj(filename, 'script', filename.replace(/^\$/, modulePath));
  discoverDependencies(file, options, dependencies);
};
matchers['^[a-zA-Z0-9_\\-\\/]+$'] = function(id, options, dependencies) {
  // TODO: only add modules to the stack if they can be loaded
  var file = new fileObj(id, 'module', modulePath + id + '.js');
  discoverDependencies(file, options, dependencies);
};

function discoverDependencies(file, options, dependencies) {
  dependencies = dependencies || { scripts: [], modules: [], warnings: [] };
  if (options.exclude.indexOf(file.name) != -1) {
    console.log('Ignoring file:'.yellow, file.filename);
    return dependencies;
  }
  try {
    var ast = getSource(file.filename);
    file.ast = ast;
    addToEnd(dependencies[file.type+'s'], file);
    console.log('Loading file:'.green, file.filename);
  } catch (error) {
    dependencies.warnings.push(file.filename);
    // either file cannot be found, or cannot be parsed
    console.log('Failed to load file:'.red, file.filename);
    return dependencies;
  }

  function walk(node) {
    if (Array.isArray(node)) {
      if (isUsing(node)) {
        var deps = extractDeps(node[2]);

        deps.forEach(function(d) {
          for (var regex in matchers) {
            if (d.match(regex)) {
              matchers[regex](d, options, dependencies);
              break;
            }
          }
        });
      } else if (isProvide(node)) {
        if (node[2][0][0] === 'function') {
          node[2].unshift(['string', file.name]);
        }
      }

      node.forEach(walk);
    }
  }

  walk(ast);

  return dependencies;
}

/*
params:

main-file
excludes: modules and scripts to exclude
module-path
current-web-path
output-file

-minimize
-jshint
-show-tree
*/

mixin(exports, {
  combine: function(filename, options) {
    options = options || {  };
    modulePath = options.path || process.cwd();
    modulePath = modulePath.replace(/\$\//,'') + '/';
    var file = new fileObj(filename, 'module', modulePath + filename + '.js');
    var dependencies = discoverDependencies(file, options);

    var outputCode = [];
    dependencies.scripts.forEach(function(script) {
      outputCode.push(jspro.gen_code(script.ast));
      outputCode.push('this.loadrunner.Script.loaded.push("' + script.name + '")');
    });
    dependencies.modules.reverse();
    dependencies.modules.forEach(function(module) {
      outputCode.push(jspro.gen_code(module.ast));
    });
    var code = outputCode.join(';');
    console.log('Output file to'.white, options.output)
    fs.writeFileSync(options.output, );
  }
});