...
function InMemoryFile(fullPath, preProcessor, builder, snippet) {
  this.fullPath = fullPath;
  this.preProcessor = preProcessor;
  this.derived = {};
  this.builder = builder;
  this.source = this.preProcess(snippet);
}
util.extend(InMemoryFile.prototype, File.prototype);
util.extend(InMemoryFile.prototype, {
  load: function() { }
});

file.createInMemoryFile = function(fullPath, preProcessor, builder, snippet) {
  var f = new InMemoryFile(fullPath, preProcessor, builder, snippet);
  files[fullPath] = f;
  return f;
}
