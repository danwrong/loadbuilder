...
util.extend(Module.prototype, {
  ...
  getAsset: function(dep, parent) {
    var asset = this.builder.matchAsset(resolveRelativePath(dep, parent ? parent.id : ''), false);
  ...
  addId: function(subTree) {
    ...
    var parent = subTree.parent.expression || subTree.parent.consequent;
...
function resolveRelativePath(dep, id) {
  var relativeParts = id.split('/'),
      depParts = dep.split('/'),
      relative = false;
